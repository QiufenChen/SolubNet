...
def Train(net, data_train, data_test, learning_rate, batch_size, max_epochs, output_freq, device):
    ...
    test_graphs = [gx[1] for gx in data_test]
    test_labels = th.tensor([gx[2] for gx in data_test]).to(device)
    ...
    def getY(gs, ps):
        ...
        for i in range(num_ps):
            ...
            p0s[i] = th.sum(net(gs[i].to(device)))
    ...
    separator = "-" * 200
    print(separator)
    print("%10s %15s %15s %15s %15s %15s %15s %15s %15s %15s %15s" %
          ("Epoch", "TrainLoss", "TrainMAE", "TrainRMSE", "TrainR2", "TrainCC", "TestMAE", "TestRMSE", "TestR2", "TestCC", "Time(s)"))
    print(separator)
    ...
    for epoch in range(max_epochs + 1):
        ...
        train_cc = Spearman(y_test, y_predict)

        scheduler.step(train_loss)

        with th.no_grad():
            y_pred, y_true = getY(test_graphs, test_labels)
            # test_loss = th.sqrt(criterionL(y_pred, y_true)) - 0.1 * criterionR(y_pred, y_true)
            # valid_epochs_loss.append(test_loss.item())
            y_pred = np.array([i.item() for i in y_pred])
            y_true = np.array([i.item() for i in y_true])
            test_mae = mean_absolute_error(y_true, y_pred)
            test_rmse = np.sqrt(mean_squared_error(y_true, y_pred))
            test_r2 = r2_score(y_true, y_pred)
            test_cc = Spearman(y_true, y_pred)

            t1 = time.time()
            dur = t1 - t0
            print("%10d %15.3f %15.3f %15.3f %15.3f %15.3f %15.3f %15.3f %15.3f %15.3f %15.3f" % 
                  (epoch, train_loss, train_mae, train_rmse, train_r2, train_cc, test_mae, test_rmse, test_r2, test_cc, dur))

            if test_mae <= MinValMae:
                MinValMae = test_mae
                BestModel = copy.deepcopy(net)
                BestEpoch = epoch
                BestTrainMAE, BestTrainRMSE, BestTrainR2, BestTrainCC=train_mae, train_rmse, train_r2, train_cc
                BestTestMAE, BestTestRMSE, BestTestR2, BestTestCC=test_mae, test_rmse, test_r2, test_cc               
    ...
    print("%15s %15s %15s %15s %15s %15s %15s %15s %15s" %
        ("BestEpoch", "TrainMAE", "TrainRMSE", "TrainR2", "TrainCC", "TestMAE", "TestRMSE", "TestR2", "TestCC"))
    print("%10d %15.3f %15.3f %15.3f %15.3f %15.3f %15.3f %15.3f %15.3f" % 
        (BestEpoch, BestTrainMAE, BestTrainRMSE, BestTrainR2, BestTrainCC, BestTestMAE, BestTestRMSE, BestTestR2, BestTestCC))
