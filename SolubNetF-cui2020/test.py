...
def visualization(graph, R, sml_id, sml, y_true, y_pred):
    ...
    plt.text(0, 0, text, fontsize=10, transform=plt.gca().transAxes)
    # plt.show()
    plt.savefig('./drug_graph/' + sml_id + ".png", format="PNG", dpi=600)
    ...
    draw.save('./drug_molecules/' + sml_id + '.png')

    # plt.subplot(2, 2, 4)
    Draw.MolToImage(mol, size=(600, 600), kekulize=True)
    Draw.ShowMol(mol, size=(600, 600), kekulize=False)
    Draw.MolToFile(mol, './drug_molecules/' + sml_id + '.png', size=(600, 600), kekulize=False)
...
def get_matrics(y_pred, y_true):
    ...
    print(separator)
    print("%15s %15s %15s" % ("Test-MAE", "Test-MSE", "Test-RMSE"))
    print("%15.4f %15.4f %15.4f " % (np.round(mae.numpy(), 4), np.round(mse.numpy(), 4), np.round(rmse.numpy(), 4)))
    print(separator)
...
if __name__ == '__main__':
    ...
    data_fn = 'dataset/' + 'test_cui_feature' + '.csv'
