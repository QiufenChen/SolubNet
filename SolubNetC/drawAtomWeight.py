...
def visualization(graph, R, id, sml):
    ...
    with open('./Graph/Klop120/'+str(id)+'.svg', 'w') as f:
        f.write(svg)
...
if __name__ == '__main__':
    ...
    FilePath = "./extended_dataset/Klop120.csv"
    ...
    model_path = project_path + '/models/solubNet5.pt'
    ...
    for i, gx in enumerate(data):
        sml = data[i][0]
        y_true = round(gx[2], 4)
        y_pred = round(th.sum(solubNet(gx[1]), dim=0).item(), 3)
        print("%5d %15.3f %15.3f" % (i, y_true, y_pred))
