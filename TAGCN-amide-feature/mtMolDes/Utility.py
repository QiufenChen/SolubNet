...
def LoadGaoData(fn, num_features, feature_str, device):
    ...
    for line in csv_reader:
        graph = ParseSMILES(line[1], num_features, feature_str, device)
        prop = float(line[2])
        data.append([line[1], graph, prop])
...
def Train(net, data_train,  data_valid, learning_rate, batch_size, max_epochs, output_freq, device):
    ...
    print("%10s %15s %15s %15s %15s %15s %15s %15s %15s %15s %15s" %
          ("Epoch", "TrainLoss", "TrainMAE", "TrainRMSE", "TrainR2", "TrainCC", "ValidMAE", "ValidRMSE", "ValidR2", "ValidCC", "Time(s)"))
    print(separator)
    ...
    for epoch in range(max_epochs + 1):
        ...
        with th.no_grad():
            ...
            print("%10d %15.3f %15.3f %15.3f %15.3f %15.3f %15.3f %15.3f %15.3f %15.3f %15.3f" % 
                  (epoch, train_loss, train_mae, train_rmse, train_r2, train_cc,valid_mae, valid_rmse, valid_r2, valid_cc, dur))
            ...
            if valid_mae <= MinValMae:
                ...
                BestTestMAE, BestTestRMSE, BestTestR2, BestTestCC=valid_mae, valid_rmse, valid_r2, valid_cc               
